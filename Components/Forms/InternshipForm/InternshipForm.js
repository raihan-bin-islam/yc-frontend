...
function InternshipForm({ draftData }) {
  // STYLE CLASSNAME
  const {
    internshipForm,
  ...
  useEffect(() => {
    ...
    unregisterInputFields(activitiesCount, false);
  }, [eduCount, activitiesCount]);
  ...
  const unregisterInputFields = (count, educational = true) => {
    ...
    if (educational) {
      // Unregister educational qualifications input fields
    ...
    // Unregister extra curricular activities input fields
    unregister(`activity[${count + 1}]`); // example: unregister institutionName[2] when eduCount == 1,
    unregister(`position[${count + 1}]`);
    unregister(`achievement[${count + 1}]`);
    unregister(`activityPeriodFrom[${count + 1}]`);
    unregister(`activityPeriodTo[${count + 1}]`);
  };
